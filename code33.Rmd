...
```{r}
#Plot raw data for variables we are interested in：

#Plot respondents' age by bar plot:
...
#Plot respondents' sex by pie chart:
...
#Plot respondents' workplace size by bar plot:
...
#Plot the response to 'How often do you consider your workload manageable?' by bar plot:
...
#Plot the Response to 'Is work distributed fairly in your workplace?' by bar plot
...
#Plot the response to stress level by bar plot:
...
#Plot the personal income group (before tax) by bar plot:
...
#Plot the the degree of qualification for the job by bar plot:
...
#Plot respondents' sense of belonging to the organization by bar plot:
...
#Plot the response to 'Are you planning on leaving your current job?' by bar plot:
...
```{r}
#citation the packages, r and rmarkdown
